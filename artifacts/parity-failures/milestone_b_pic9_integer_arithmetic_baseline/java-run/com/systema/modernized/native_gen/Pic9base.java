...
public class Pic9base {
    ...
    private void main_process() {
        ...
        skipToNextSentence = false;
        if (!skipToNextSentence) { a = 15; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { b = 25; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { c = 0; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { c = (int)((a + b) % 10000L); }
        skipToNextSentence = false;
        {
        ...
        skipToNextSentence = false;
        if (!skipToNextSentence) { c = (int)((b - a) % 10000L); }
        skipToNextSentence = false;
        {
        writeBytes(get_ws_group_bytes());
        System.out.write(10);

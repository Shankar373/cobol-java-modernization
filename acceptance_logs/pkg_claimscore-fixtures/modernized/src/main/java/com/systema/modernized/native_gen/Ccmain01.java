...
public class Ccmain01 {
    ...
    private void main_section() {
        ...
        if (!skipToNextSentence) { System.out.println("CLAIMSCORE ENTERPRISE NIGHTLY BATCH"); }
        if (!skipToNextSentence) {
            Ccload01 ccload01_1 = new Ccload01();
                    ccload01_1.execute();
                    return_code = ccload01_1.return_code;
        }
        // UNSUPPORTED: UNKNOWN
        if (ws_rc !=0) {
        if (!skipToNextSentence) { return_code = 12; }
        if (true) { programExited = true; return; }
        }
        if (!skipToNextSentence) {
            Ccproc01 ccproc01_2 = new Ccproc01();
                    ccproc01_2.execute();
                    return_code = ccproc01_2.return_code;
        }
        // UNSUPPORTED: UNKNOWN
        if (ws_rc !=0) {
        if (!skipToNextSentence) { return_code = 12; }
        ...
        if (!skipToNextSentence) {
            Ccrept01 ccrept01_3 = new Ccrept01();
                    ccrept01_3.execute();
                    return_code = ccrept01_3.return_code;
        }
        // UNSUPPORTED: UNKNOWN
        if (ws_rc !=0) {
        if (!skipToNextSentence) { return_code = 12; }
        ...
        if (!skipToNextSentence) { System.out.println("CLAIMSCORE NIGHTLY BATCH COMPLETED"); }

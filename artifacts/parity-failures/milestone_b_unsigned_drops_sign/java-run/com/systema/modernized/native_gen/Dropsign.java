...
public class Dropsign {
    ...
    public byte[] get_ws_group_bytes() {
        byte[] c_0 = formatSigned(a, 3, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
    ...
    private void populate_ws_group(String line) {
        ...
        ws_group = line;
        if (line.length() >= 3) {
            String val = line.substring(0, 3).trim();
    ...
    private void main_process() {
        ...
        if (!skipToNextSentence) { a = 0; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { a = 0; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { a = -5; }
        skipToNextSentence = false;
        {

...
public class Pcorr {
    ...
    public int ws_x = 10;
    public int ws_y = 20;
    public String ws_z = "HELLO";
    public String ws_b = "";
    public String ws_w = "WORLD";
    ...
    public byte[] get_ws_a_bytes() {
        byte[] c_0 = formatSigned(ws_x, 2, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_1 = formatSigned(ws_y, 2, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_2 = ws_z.getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
    ...
    public byte[] get_ws_b_bytes() {
        byte[] c_0 = formatSigned(ws_x, 2, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_1 = formatSigned(ws_y, 2, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_2 = ws_w.getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] res = new byte[c_0.length + c_1.length + c_2.length];
        System.arraycopy(c_0, 0, res, 0, c_0.length);
        System.arraycopy(c_1, 0, res, 0 + c_0.length, c_1.length);
        System.arraycopy(c_2, 0, res, 0 + c_0.length + c_1.length, c_2.length);
    ...
    private void populate_ws_a(String line) {
        if (line == null) line = "";
        ws_a = line;
        if (line.length() >= 2) {
            String val = line.substring(0, 2).trim();
            ws_x = val.isEmpty() ? 0 : Integer.parseInt(val);
        }
        if (line.length() >= 4) {
            String val = line.substring(2, 4).trim();
            ws_y = val.isEmpty() ? 0 : Integer.parseInt(val);
        }
        if (line.length() >= 9) {
            String val = line.substring(4, 9).trim();
            ws_z = val;
        }
    }

    private void populate_ws_b(String line) {
        if (line == null) line = "";
        ws_b = line;
        if (line.length() >= 2) {
            String val = line.substring(0, 2).trim();
    ...
    private void main_process() {
        ...
        skipToNextSentence = false;
        if (!skipToNextSentence) {
            ws_x = (int)((ws_x + ws_x) % 100L);
                    ws_y = (int)((ws_y + ws_y) % 100L);
        }
        skipToNextSentence = false;
        if (!skipToNextSentence) {
            {

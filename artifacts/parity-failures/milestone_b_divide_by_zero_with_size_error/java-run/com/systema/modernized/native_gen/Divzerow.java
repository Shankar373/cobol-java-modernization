...
public class Divzerow {
    ...
    public byte[] get_ws_group_bytes() {
        ...
        byte[] c_4 = formatSigned(c, 4, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
    ...
    private void populate_ws_group(String line) {
        ...
        if (line.length() >= 14) {
            ...
            c = val.isEmpty() ? 0 : Integer.parseInt(val);
        }
    }
    ...
    private void main_process() {
        ...
        skipToNextSentence = false;
        if (!skipToNextSentence) { b = 0; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { c = 5; }
        skipToNextSentence = false;
        {
        com.systema.modernized.runtime.CobolArithmetic.checkPrecision(4, 0);
            if (((b) == 0) || checkSizeError((a / b), 4, false)) {
            {
        writeBytes("DIVBYZERO".getBytes(java.nio.charset.StandardCharsets.ISO_8859_1));
        System.out.write(10);
        System.out.flush();
    }
        } else {

...
public class Fpfast {
    ...
    public String ws_vars = "";
    public int ws_int = 0;
    public long ws_long = 0;
    ...
    public byte[] get_ws_vars_bytes() {
        byte[] c_0 = formatSigned(ws_int, 9, true).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_1 = formatSigned(ws_long, 15, true).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
    ...
    private void populate_ws_vars(String line) {
        if (line == null) line = "";
        ws_vars = line;
        if (line.length() >= 9) {
            String val = line.substring(0, 9).trim();
            ws_int = (int) parseSignedLong(val);
        }
        if (line.length() >= 24) {
            String val = line.substring(9, 24).trim();
            ws_long = (long) parseSignedLong(val);
        }
    }
    ...
    private void main_process() {
        ...
        if (!skipToNextSentence) { ws_int = 123456789; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { ws_long = -987654321012345L; }
        skipToNextSentence = false;
        if (!skipToNextSentence) {
            {
                    writeBytes(new com.systema.modernized.runtime.CobolNumeric(java.math.BigDecimal.valueOf(ws_int), new com.systema.modernized.runtime.CobolNumericSpec(true, 9, 0, com.systema.modernized.runtime.CobolUsage.DISPLAY, com.systema.modernized.runtime.CobolSignPosition.TRAILING, false)).toDisplayString().getBytes(java.nio.charset.StandardCharsets.ISO_8859_1));
                    System.out.write(10);
                    System.out.flush();
                }
        }
        ...
        if (!skipToNextSentence) {
            {
                    writeBytes(new com.systema.modernized.runtime.CobolNumeric(java.math.BigDecimal.valueOf(ws_long), new com.systema.modernized.runtime.CobolNumericSpec(true, 15, 0, com.systema.modernized.runtime.CobolUsage.DISPLAY, com.systema.modernized.runtime.CobolSignPosition.TRAILING, false)).toDisplayString().getBytes(java.nio.charset.StandardCharsets.ISO_8859_1));
                    System.out.write(10);
                    System.out.flush();
                }
        }
        skipToNextSentence = false;
        if (!skipToNextSentence) { ws_int = (int)((ws_int + 10) % 1000000000L); }
        skipToNextSentence = false;
        if (!skipToNextSentence) { ws_long = (long)((ws_long - 100L) % 1000000000000000L); }
        skipToNextSentence = false;
        if (!skipToNextSentence) {
            {
                    writeBytes(new com.systema.modernized.runtime.CobolNumeric(java.math.BigDecimal.valueOf(ws_int), new com.systema.modernized.runtime.CobolNumericSpec(true, 9, 0, com.systema.modernized.runtime.CobolUsage.DISPLAY, com.systema.modernized.runtime.CobolSignPosition.TRAILING, false)).toDisplayString().getBytes(java.nio.charset.StandardCharsets.ISO_8859_1));

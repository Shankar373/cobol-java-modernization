...
public class Sizeerr {

    public int return_code = 0;
    public int ws_tgt = 50;
    ...
    private void main_process() {
        ...
        if (!skipToNextSentence) {
            {
                    if (checkSizeError((ws_tgt + 60), 2, false)) {
                        if (!skipToNextSentence) {
                        {
                                writeBytes("OVERFLOW".getBytes(java.nio.charset.StandardCharsets.ISO_8859_1));
                                writeBytes(end_add.getValue().getBytes(java.nio.charset.StandardCharsets.ISO_8859_1));
                                System.out.write(10);
                                System.out.flush();
                            }
                    }
                        if (!skipToNextSentence) {
                        {
                                writeBytes(new com.systema.modernized.runtime.CobolNumeric(java.math.BigDecimal.valueOf(ws_tgt), new com.systema.modernized.runtime.CobolNumericSpec(false, 2, 0, com.systema.modernized.runtime.CobolUsage.DISPLAY, com.systema.modernized.runtime.CobolSignPosition.TRAILING, false)).toDisplayString().getBytes(java.nio.charset.StandardCharsets.ISO_8859_1));
                                System.out.write(10);
                                System.out.flush();
                            }
                    }
                    } else {
                        ws_tgt = (int)((ws_tgt + 60) % 100L);

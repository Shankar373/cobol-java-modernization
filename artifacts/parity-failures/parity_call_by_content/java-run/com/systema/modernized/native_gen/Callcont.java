...
public class Callcont {

    public int return_code = 0;
    public int ws_original = 999;
    ...
    private void main_process() {
        ...
        if (!skipToNextSentence) {
            Mutator mutator_1 = new Mutator(this);
                    mutator_1.ls_param = ws_original;
                    mutator_1.execute();
                    return_code = mutator_1.return_code;
                    ws_original = mutator_1.ls_param;
        }
        if (!skipToNextSentence) {
            {
                    writeBytes(new com.systema.modernized.runtime.CobolNumeric(java.math.BigDecimal.valueOf(ws_original), new com.systema.modernized.runtime.CobolNumericSpec(false, 6, 0, com.systema.modernized.runtime.CobolUsage.DISPLAY, com.systema.modernized.runtime.CobolSignPosition.TRAILING, false)).toDisplayString().getBytes(java.nio.charset.StandardCharsets.ISO_8859_1));
                    System.out.write(10);
                    System.out.flush();
                }
        }
    ...
    public static class Mutator {
        private final Callcont parent;
        public Mutator(Callcont parent) {
            this.parent = parent;
        }
    
        public int return_code = 0;
        public int ls_param = 0;
        {  // Initialise redefines values
        }
        ...
        private void main_process() {
            skipToNextSentence = false;
            if (!skipToNextSentence) { ls_param = (int)((ls_param + 1) % 1000000L); }
            skipToNextSentence = false;
            if (!skipToNextSentence) { if (true) { programExited = true; return; } }
        }

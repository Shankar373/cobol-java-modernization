...
public class Divscale {
    ...
    private void populate_ws_group(String line) {
        ...
        if (line.length() >= 13) {
            String val = line.substring(7, 13).trim();
            b.assign(val.isEmpty() ? BigDecimal.ZERO : new BigDecimal(val).movePointLeft(2));
        }
        if (line.length() >= 14) {
            String val = line.substring(13, 14).trim();
            filler_2 = val;
        }
        if (line.length() >= 20) {
            String val = line.substring(14, 20).trim();
    ...
    private void main_process() {
        ...
        skipToNextSentence = false;
        if (!skipToNextSentence) { a.assign(new BigDecimal("10.00"), com.systema.modernized.runtime.CobolRoundingMode.TRUNCATION, com.systema.modernized.runtime.SizeErrorPolicy.UNCHECKED); }
        skipToNextSentence = false;
        if (!skipToNextSentence) { b.assign(new BigDecimal("3.00"), com.systema.modernized.runtime.CobolRoundingMode.TRUNCATION, com.systema.modernized.runtime.SizeErrorPolicy.UNCHECKED); }
        skipToNextSentence = false;
        ...
        skipToNextSentence = false;
        {
            com.systema.modernized.runtime.CobolArithmetic.checkPrecision(6, 2);
            if (!((b.getValue()).compareTo(BigDecimal.ZERO) == 0)) {
                c.assign(com.systema.modernized.runtime.CobolArithmetic.divide(a.getValue(), b.getValue()), com.systema.modernized.runtime.CobolRoundingMode.TRUNCATION, com.systema.modernized.runtime.SizeErrorPolicy.UNCHECKED);
            }
        }
        skipToNextSentence = false;
        {

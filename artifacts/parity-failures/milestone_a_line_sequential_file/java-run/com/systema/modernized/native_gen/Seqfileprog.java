...
public class Seqfileprog {
    ...
    private BufferedReader out_file_reader;
    private BufferedWriter out_file_writer;
    ...
    private void open_out_file(String mode) {
        try {
            ...
            if ("INPUT".equalsIgnoreCase(mode)) {
                out_file_reader = Files.newBufferedReader(Paths.get(resolve_path_out_file()));
            } else if ("OUTPUT".equalsIgnoreCase(mode)) {
                java.nio.file.Path parent = Paths.get(resolve_path_out_file()).getParent();
                if (parent != null) Files.createDirectories(parent);
                out_file_writer = Files.newBufferedWriter(Paths.get(resolve_path_out_file()));
            }
        } catch (IOException e) {
    ...
    private boolean read_out_file() {
        try {
            if (out_file_reader == null) return false;
            String line = out_file_reader.readLine();
            if (line == null) {
                return false;
            } else {
                String val_out_rec = (line.length() >= 20) ? line.substring(0, 20) : (line.length() > 0 ? padString(line.substring(0), 20) : padString("", 20));
                out_rec = val_out_rec;
            }
            return true;
        } catch (IOException e) {
    ...
    private void write_out_file() {
        try {
            if (out_file_writer == null) return;
            out_file_writer.write(String.format("%-20s", out_rec));
    ...
    private void close_out_file() {
        try {
            if (out_file_reader != null) { out_file_reader.close(); out_file_reader = null; }
            if (out_file_writer != null) { out_file_writer.close(); out_file_writer = null; }
        } catch (IOException e) {
        }
    }
    ...
    private void main_process() {
        ...
        if (!skipToNextSentence) { out_rec = padString(String.valueOf("HELLO WORLD 1"), 20); }
        if (!skipToNextSentence) { write_out_file(); }
        if (!skipToNextSentence) { out_rec = padString(String.valueOf("HELLO WORLD 2"), 20); }
        if (!skipToNextSentence) { write_out_file(); }
        if (!skipToNextSentence) { close_out_file(); }
        if (!skipToNextSentence) {
            {
                    writeBytes("FILE WRITTEN".getBytes(java.nio.charset.StandardCharsets.ISO_8859_1));
                    System.out.write(10);
                    System.out.flush();
                }
        }

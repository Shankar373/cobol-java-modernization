...
public class Negsub {
    ...
    public byte[] get_ws_group_bytes() {
        ...
        byte[] c_2 = formatSigned(x, 4, true).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
    ...
    private void main_process() {
        ...
        skipToNextSentence = false;
        if (!skipToNextSentence) { x = 0; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { x = (int)((com.systema.modernized.runtime.CobolArithmetic.subtract(BigDecimal.valueOf(a), new BigDecimal("1"))).longValue() % 10000L); }
        skipToNextSentence = false;
        {
        ...
        skipToNextSentence = false;
        if (!skipToNextSentence) { x = (int)((com.systema.modernized.runtime.CobolArithmetic.subtract(BigDecimal.valueOf(a), (new BigDecimal("1")).negate())).longValue() % 10000L); }
        skipToNextSentence = false;
        {
        writeBytes(get_ws_group_bytes());
        System.out.write(10);
        ...
        skipToNextSentence = false;
        if (!skipToNextSentence) { x = (int)((a - 1) % 10000L); }
        skipToNextSentence = false;
        {
        writeBytes(get_ws_group_bytes());
        System.out.write(10);

...
public class Posover {
    ...
    public byte[] get_ws_group_bytes() {
        byte[] c_0 = formatSigned(a, 2, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_1 = filler_1.getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_2 = formatSigned(b, 2, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
    ...
    private void populate_ws_group(String line) {
        ...
        if (line.length() >= 5) {
            String val = line.substring(3, 5).trim();
    ...
    private void main_process() {
        ...
        skipToNextSentence = false;
        if (!skipToNextSentence) { a = 99; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { b = 10; }
        skipToNextSentence = false;
        {
        if (checkSizeError((5 + a), 2, false)) {
            {

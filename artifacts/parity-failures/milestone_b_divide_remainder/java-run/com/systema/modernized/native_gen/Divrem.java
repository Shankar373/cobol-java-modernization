...
public class Divrem {
    ...
    public byte[] get_ws_group_bytes() {
        byte[] c_0 = formatSigned(a, 4, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_1 = filler_1.getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_2 = formatSigned(b, 4, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_3 = filler_2.getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_4 = formatSigned(q, 4, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_5 = filler_3.getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] c_6 = formatSigned(r, 4, false).getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
        byte[] res = new byte[c_0.length + c_1.length + c_2.length + c_3.length + c_4.length + c_5.length + c_6.length];
        ...
        System.arraycopy(c_5, 0, res, 0 + c_0.length + c_1.length + c_2.length + c_3.length + c_4.length, c_5.length);
        System.arraycopy(c_6, 0, res, 0 + c_0.length + c_1.length + c_2.length + c_3.length + c_4.length + c_5.length, c_6.length);
        return res;
    }
    private void populate_ws_group(String line) {
        ...
        if (line.length() >= 4) {
            ...
            a = val.isEmpty() ? 0 : Integer.parseInt(val);
        }
        if (line.length() >= 5) {
            String val = line.substring(4, 5).trim();
            filler_1 = val;
        }
        if (line.length() >= 9) {
            String val = line.substring(5, 9).trim();
            b = val.isEmpty() ? 0 : Integer.parseInt(val);
        }
        if (line.length() >= 10) {
            String val = line.substring(9, 10).trim();
            filler_2 = val;
        }
        if (line.length() >= 14) {
            String val = line.substring(10, 14).trim();
        ...
        if (line.length() >= 19) {
            String val = line.substring(15, 19).trim();
            r = val.isEmpty() ? 0 : Integer.parseInt(val);
        }
    }
    ...
    private void main_process() {
        ...
        skipToNextSentence = false;
        if (!skipToNextSentence) { a = 10; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { b = 3; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { q = 0; }
        skipToNextSentence = false;
        if (!skipToNextSentence) { r = 0; }
        skipToNextSentence = false;
        if (!skipToNextSentence) {
            {
                    com.systema.modernized.runtime.CobolArithmetic.checkPrecision(4, 0);
                        if (!((b) == 0)) {
                        q = (int)((a / b) % 10000L);
                    }
                }
